...
namespace VanguardLTE\Http\Controllers\Web\Frontend
{
    ...
    class GamesController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        /*    if( !\Illuminate\Support\Facades\Auth::check() && false ) 
            ...
            // Disabled redirect to allow landing page to load
            ...
            if($category1 != '') {
                return view('frontend.' . $frontend . '.games.category', compact('games', 'category1', 'cat1', 'categories', 'currentSliderNum', 'title', 'body', 'keywords', 'description', 'jpgs', 'shop', 'devices', 'tournament', 'is_game_page', 'jpgSum', 'gamestat', 'depositlist'));
            }
        ...
        public function netflix(\Illuminate\Http\Request $request)
        {
            if( \Illuminate\Support\Facades\Auth::check() && !auth()->user()->hasRole('user') ) 
            {
                return redirect()->route('backend.dashboard');
            ...
            $categoryList = [
                ['key' => 'hot', 'title' => trans('app.hot_game')],
                ['key' => 'new', 'title' => trans('app.new')],
                ['key' => 'jackpot', 'title' => 'Jackpot'],
                ['key' => 'slots', 'title' => trans('app.slots')],
                ['key' => 'pragmatic', 'title' => 'Pragmatic Play'],
                ['key' => 'egt', 'title' => 'EGT'],
                ['key' => 'netent', 'title' => 'NetEnt'],
                ['key' => 'playtech', 'title' => 'Playtech'],
                ['key' => 'wazdan', 'title' => 'Wazdan'],
                ['key' => 'ka-gaming', 'title' => 'Ka-Gaming']
            ];
            ...
            foreach ($categoryList as $catInfo) {
                ...
                try {
                    if ($catKey == 'hot') {
                        if (\VanguardLTE\Lib\GetHotNewMyGames::get_hot_games(true)) {
                            $hot_games = \VanguardLTE\Lib\GetHotNewMyGames::get_hot_games();
                            if (count($hot_games)) {
                                $games = $games->whereIn('id', $hot_games);
                            } else {
                        ...
                        if (\VanguardLTE\Lib\GetHotNewMyGames::get_new_games(true)) {
                            $new_games = \VanguardLTE\Lib\GetHotNewMyGames::get_new_games();
                            if (count($new_games)) {
                                $games = $games->whereIn('id', $new_games);
                            } else {
                        ...
                        $cat = \VanguardLTE\Category::where(['href' => $catKey])->first();
                        if ($cat) {
                            $categories = \VanguardLTE\Category::where(['parent' => $cat->id])->pluck('id')->toArray();
                            $categories[] = $cat->id;
                            $games = $games->whereRaw('original_id IN (SELECT game_id FROM "w_game_categories" WHERE category_id IN(' . implode(',', $categories) . '))');
                        } else {
                            continue;
                    ...
                    $gameList = $games->take(20)->get();
                    
                    if ($gameList && count($gameList) > 0) {
                        $categoriesWithGames[] = [
                            'title' => $catTitle,
                            'key' => $catKey,
                            'games' => $gameList
            ...
            $categories = false;
            $cat_ids = \VanguardLTE\GameCategory::whereIn('game_id', \VanguardLTE\Game::where(['view' => 1, 'shop_id' => $shop_id])->pluck('original_id'))->groupBy('category_id')->pluck('category_id');
            if( count($cat_ids) ) 
            {
                $categories = \VanguardLTE\Category::whereIn('id', $cat_ids)->orderBy('position', 'ASC')->get();
            }
            ...
            return view('frontend.' . $frontend . '.games.netflix', compact('categoriesWithGames', 'categories', 'title', 'body', 'keywords', 'description', 'shop', 'category1', 'cat1', 'currentSliderNum', 'jpgs', 'jpgSum', 'devices', 'tournament', 'is_game_page', 'gamestat', 'depositlist'));
        }
        ...
        public function bonuses()
        {
            ...
            if( $shop_id && $shop ) 
            {
                $frontend = $shop->frontend ?: 'Default';
            }
            $games = \VanguardLTE\Game::where([
                'view' => 1, 
                'shop_id' => $shop_id
            ...
            $faqs = \VanguardLTE\Faq::orderBy('position', 'ASC')->get();
            $bonuses = [];
            if($shop) {
                $bonuses = $shop->getBonusesList();
            }
            return view('frontend.' . $frontend . '.pages.bonuses', compact('currentSliderNum', 'category1', 'categories', 'bonuses', 'shop'));
        }
