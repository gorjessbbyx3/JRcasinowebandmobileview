...
public function postLogin(\VanguardLTE\Http\Requests\Auth\LoginRequest $request, \VanguardLTE\Repositories\Session\SessionRepository $sessionRepository)
{
    ...
    return $this->handleUserWasAuthenticated($request, $throttles, $user);
                ...
                {
                        $isDemo = $request->input('is_demo', 0);
                        
                        if ($isDemo) {
                                $username = 'demo_' . strtolower(substr(md5(uniqid(mt_rand(), true)), 0, 8));
                                $password = substr(str_shuffle('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'), 0, 10);
                                $uniqueUsername = $this->checkUsername($username);
                                
                                $data = [
                                        'username' => $uniqueUsername,
                                        'email' => $uniqueUsername . '@demo.local',
                                        'password' => $password
                                ];
                                ...
                                try {
                                        ...
                                        \Log::warning('Event firing failed during demo registration: ' . $e->getMessage());
                                }
                                
                                \Auth::login($user, true);
                                
                                $message = "Demo account created successfully! Please save your login credentials.";
                                ...
                                return redirect()->route('frontend.game.list')->with('success', $message);
                        } else {
